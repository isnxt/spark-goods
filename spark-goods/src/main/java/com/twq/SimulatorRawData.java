...
class DataSimulator implements Runnable{
	...
	//url指向要访问的数据库名spark
	public static final String url = "jdbc:mysql://127.0.0.1:3306/spark?characterEncoding=utf8&useSSL=true";
	public static final String user="root";
	public static final String password="root";

	private int GOOD_NUM=10;    //商品数
	private int USER_NUM=500;    //用户数
	private int MSG_NUM=50;     //每次产生的消息数
...
//			ServerSocket serSocket=new ServerSocket(9999);
			...
			try{
				...
				String sql="insert into rawData(userID,itemID,browser_num,stay_time,collect,buy_num,scores) " +
						"values(?,?,?,?,?,?,?)";
...
//					PrintWriter pw=new PrintWriter(os);
					for(int i=0;i<msgNum;i++) {
						...
						int itemID=(msgNum%10+1);
						int userID=(time*msgNum+msgNum+1);
...
//						int userID=(r.nextInt(USER_NUM)+1);
						...
						//检查是否存在
						ResultSet rs = stmt.executeQuery("SELECT * FROM rawdata WHERE userID="+userID+" AND itemID="+itemID);
						rs.last();
						int rows = rs.getRow();
						if (rows != 0)
						{
						...
						sb.append("::");
						sb.append(score);
						System.out.println(i+".写入成功:"+sb.toString());
